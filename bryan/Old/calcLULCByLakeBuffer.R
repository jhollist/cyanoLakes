...
LULCcodes<-data.frame(Code=c(0,11,12,21,22,23,24,31,41,42,43,52,71,81,82,90,95,NA)) #these codes in the lower 48-not sure what zero is.
NoValues<-c(51,72,73,74)  #these codes apply to alaska only
...
#LULC is the impervious cover for all lower 48 states.  Reduce to bbox(Lake) + max(buffer) + 1000m
...
Mask<-mask(Grid, Buf4)  #extract impervious cover pixels in the buffer (all others changed to NA)
a<-data.frame(table(getValues(Mask),useNA='ifany'))  #the NA are pixels outside of bbox of buffer.  Values of "127" are the real NA

a<-merge(LULCcodes,a,by.x='Code',by.y='Var1',all.x=T)
a[is.na(a$Freq),'Freq']<-0
